...
// Horizontal color bands based on world position

float4 color1     = float4(1.0, 0.3, 0.3, 1);
float4 color2     = float4(0.3, 1.0, 0.3, 1);
float4 color3     = float4(0.3, 0.3, 1.0, 1);
float4 tex_trans  = float4(0, 0, 1, 1);
float  band_scale = 3.0;
float  ambient    = 0.2;
...
float4 ps(psIn input) : SV_TARGET {
	...
	// Select band color based on world Y
	float y = frac(input.world_pos.y * band_scale);
	float4 band_color;
	if (y < 0.333) {
		band_color = color1;
	} else if (y < 0.666) {
	...
	float3 final_color = albedo.rgb * band_color.rgb * diffuse;
