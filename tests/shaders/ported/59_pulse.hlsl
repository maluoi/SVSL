...
// Animated pulsing glow effect using time

float4 color       = float4(1, 1, 1, 1);
float4 glow_color  = float4(1.0, 0.5, 0.2, 1);
...
float4 ps(psIn input) : SV_TARGET {
	...
	// Animated pulse using sine wave
	float pulse = sin(time * pulse_speed) * 0.5 + 0.5;
	float glow  = lerp(glow_min, glow_max, pulse);
	...
	// Add glow
	float3 final_color = albedo.rgb * diffuse + glow_color.rgb * glow;
