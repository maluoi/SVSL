...
// Cel/toon shading with discrete lighting bands

float4 color        = float4(1, 1, 1, 1);
float4 tex_trans    = float4(0, 0, 1, 1);
float4 outline_col  = float4(0.1, 0.1, 0.15, 1);
float  bands        = 3.0;
float  outline_size = 0.03;
...
float4 ps(psIn input) : SV_TARGET {
	...
	float lighting = floor(ndotl * bands + 0.5) / bands;
	lighting       = max(lighting, 0.2); // Minimum ambient

	// Edge detection for outline
	float ndotv = saturate(dot(normal, view_dir));
	float edge  = smoothstep(0.0, outline_size, ndotv);

	float4 albedo = tex.Sample(tex_sampler, input.uv) * input.color;
	float3 toon   = albedo.rgb * lighting;

	// Blend with outline color at edges
	float3 final = lerp(outline_col.rgb, toon, edge.xxx);
