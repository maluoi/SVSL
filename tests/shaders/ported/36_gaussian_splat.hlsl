...
float  alpha_mult;       // Alpha multiplier for alpha-to-coverage
...
float4 ps(psIn input) : SV_TARGET {
	...
	// Apply alpha multiplier for alpha-to-coverage
	float out_alpha = saturate(alpha * alpha_mult);

	// Colors are already in linear space (converted at load time)
	// sRGB framebuffer will convert to sRGB for correct display
	return float4(input.color * alpha, out_alpha);
}
